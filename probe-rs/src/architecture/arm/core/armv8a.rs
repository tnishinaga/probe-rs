...
impl<'probe> Armv8a<'probe> {
    ...
    fn write_cpu_memory_aarch64_fast(&mut self, address: u64, data: &[u8]) -> Result<(), Error> {
        ...
        for byte in head {
            self.write_word_8(address, *byte)?;
            address += 1;
        }

        // write aligned part
        self.write_cpu_memory_aarch64_fast_inner(address, aligned)?;
    ...
    fn read_cpu_memory_aarch64_fast(&mut self, address: u64, data: &mut [u8]) -> Result<(), Error> {
        ...
        // read unaligned part
        for byte in head {
            *byte = self.read_word_8(address)?;
            address += 1;
        }

        // read aligned part
        self.read_cpu_memory_aarch64_fast_inner(address, aligned)?;
